# Assess Code Review — Execution Guide
...
Before anything else, run the `ToolSearch` `select:...` load (above). If the Bitbucket tools do not resolve (MCP not connected in this session), STOP immediately and tell Brian the Bitbucket MCP isn't available and to connect it — do NOT attempt git/API fallbacks. This is a hard precondition: every downstream step depends on these tools.

---

## Step A. Resolve the target PR

> **Defaults (recap from the preamble at the top of this file):** `repository = "drovacorp/interface"`, target `master`. Step A and Step F both consume this `repository` value; keep the literal here in sync with the preamble so a future platform swap is a two-line edit.

1. If `[PR-ID]` is provided → use the Defaults-block `repository`, `prId = <arg>`. Confirm it exists via `get_pull_request`; if it 404s, stop and tell Brian the id wasn't found rather than guessing.
