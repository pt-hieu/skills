...
Source of truth for Brian's task-intake workflow — a gated pipeline that takes a new requirement from intake to a green-lit, pitched plan ready for implementation. Walks a fresh requirement through gated steps so no critical phase (skill scan, challenge, pitch) is skipped.
...
- **Action**: in a single message, call `TaskCreate` once per task in the overview below, in order. Use each task's **subject** verbatim; write each **description** as a one-line pointer back to the spec file: `Execute per § "Task N — <title>" of <absolute path of references/task-specs.md, sibling of this SKILL.md> — re-read that section and pass its Gate before completing.` Then call `TaskUpdate` to wire `addBlockedBy` where order genuinely matters (`A←B` means B is blocked by A):
...
## Pipeline overview (subjects + gates)

The 12 tasks to register, with the gate that clears each. Full Goal / Action / Gate for every task lives in `references/task-specs.md` — re-read a task's section there when you pick it up.

| # | Subject | Gate (task clears when…) |
| --- | --- | --- |
| 1 | `Enter plan mode` | plan mode is active |
| 2 | `Explore — gather Phase-1 findings` | concrete file paths, reusable utilities, existing patterns (or root cause + defect class on the bug path) written down |
| 3 | `Historian — gather prior intent from git history and ticket tracker` | historian report in hand — commit timeline, linked tickets with "why" quotes (or a no-tracker note), themes, implications |
| 4 | `Interrogate — close architecture-level ambiguity` | an `AskUserQuestion` round answered (or a no-ambiguity note), remaining unknowns won't change the approach |
| 5 | `Skill scan — enumerate and apply` | scan written down and every relevant skill applied |
| 6 | `Run Plan agent — produce detailed implementation plan` | a Plan-agent return is in hand and its verbatim output is written to the plan file on disk |
| 7 | `Write the plan file` | plan file re-read end-to-end; Context/Prior-intent/Skills-to-use checks (a)–(d) all pass |
| 8 | `Challenge the plan file` | challenge round complete and the plan file on disk updated |
| 9 | `Design the Test design section of the plan file` | `test-designer` returns `verification: PASS` and a `## Test design` section sits between Skills to use and Verification |
| 10 | `Pitch the plan to Brian` | pitch is posted |
| 11 | `Verify plan coherence and inject post-implementation protocol` | `plan-verifier` returns `verification: PASS` and a `protocol:` line; `## Post-implementation protocol` is the final section |
