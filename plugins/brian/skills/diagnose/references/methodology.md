...
- The user-turn input already reads, on inspection, as carrying historian findings — prior intent or a historian's report, however it's presented — that cover every path under analysis. On this branch, state in your own words that you're reusing that prior intent and where it came from, then proceed. This protects standalone re-runs of diagnose against a plan that already carries prior-intent findings (the post-kickoff path).
...
**Empty-history fallback** — if the historian report contains no meaningful commits or tickets (tracker absence or empty timeline), state in your own words that there was no prior history and that the bedrock candidate is therefore a missing abstraction or pattern not yet built, then proceed; empty history is itself a §4 bedrock signal, not a §2 failure. State this plainly enough that its meaning carries across rounds — a later-round reader should be able to tell from what you wrote that history was checked and came up empty, without needing to re-invoke the historian.
