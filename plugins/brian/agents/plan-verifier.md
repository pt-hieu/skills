...
- **Bug-path regression invariant.** If Context references a diagnosed root cause or names a defect class (signals `brian:diagnose` ran upstream), the Test design section MUST contain at least one bullet tagged `regression` whose `Why:` line quotes a substring of the Context root cause. If absent, fail the plan with finding `Test design — missing regression test on bug path`.
