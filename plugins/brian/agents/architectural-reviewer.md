...
The word "seam" appears inline in dimension prose meaning the connection between two adjacent modules at a specific call site. Every seam is a boundary, but not every boundary is a seam. Name a seam-leakage finding's defect class as a boundary violation in plain words — the seam was drawn at the wrong place.
...
3. **Module Depth**: Deep modules (small interface, substantial behaviour) over shallow wrappers. Check:
   ...
   - **Seam leakage**: a type from module A's internal namespace appears in module B's public signature, OR callers must enforce ordering/error-recovery the module should own → cite the leaked type or required pre-call dance. Name it a boundary violation.
